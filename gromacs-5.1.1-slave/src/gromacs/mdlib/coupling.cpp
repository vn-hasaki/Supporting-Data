 ...
 */
    ...
    real tmpx = vrescale_sumnoises(ndeg-1, step, &rnd_count, seed, RND_SEED_VRESCALE);
    real tmprr = rr*rr;
    real tmp1 = sigma*(tmpx + tmprr);
    real tmp2 = (1.0 - factor)*(tmp1/ndeg-kk);
    real tmp3 = 2.0*rr*sqrt(kk*sigma/ndeg*(1.0 - factor)*factor);
    ekin_new = kk + tmp2 + tmp3;
                     ...
                     double therm_integral[])
{
    ...
    opts = &ir->opts;
    //printf("-----------------------------vrescale_tcoupl:opts->ngtc: %d-----------------------------\n", opts->ngtc);
    for (i = 0; (i < opts->ngtc); i++)
    {
        if (ir->eI == eiVV)
        {
