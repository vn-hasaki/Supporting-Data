...
extern void SLAVE_FUN(nbnxn_kernel_ref_slave());
extern void SLAVE_FUN(nbnxn_kernel_ElecQSTab_VdwLJ_VgrpF_ref_slave());

int                nnbl;
nbnxn_pairlist_t **nbl;
nbnxn_pairlist_t   *nbl_s;
nbnxn_atomdata_output_t *out;
real                    *fshift_p;
int                coult;
int                vdwt;
int                nb;
const nbnxn_atomdata_t   *nbat_s;
const interaction_const_t *ic_s;
rvec                       *shift_vec_s;
int                         force_flags_s;
int                         clearF_s;
real                       *fshift_s;
                 ...
                 real                       *Vvdw)
{
    ...
    nbat_s = nbat;
    ic_s = ic;
    shift_vec_s = shift_vec;
    force_flags_s = force_flags;
    clearF_s = clearF;
    fshift_s = fshift;
...
// #pragma omp parallel for schedule(static) num_threads(nthreads)
    // printf("---------------------------nnbl is %d---------------------------------\n", nnbl);
    // athread_init();
    // printf("Start slave fun......");
    // athread_spawn(nbnxn_kernel_ref_slave, 0);
    // athread_join();
    // athread_halt();
    // printf("End slave fun......");
    ...
    for (nb = 0; nb < nnbl; nb++)
    {
        ...
        {
            //printf("---------------------------p_nbk_c_noener, coult: %d, vdwt: %d---------------------------------\n", coult, vdwt);
            /* Don't calculate energies */
            athread_init();
            //printf("Start slave fun......\n");
            athread_spawn(nbnxn_kernel_ElecQSTab_VdwLJ_VgrpF_ref_slave, 0);
            athread_join();
            athread_halt();
            //printf("End slave fun......\n");
        ...
        else if (out->nV == 1)
        {
            ...
            out->Vvdw[0] = 0;
            out->Vc[0]   = 0;
            //printf("---------------------------p_nbk_c_ener, coult: %d, vdwt: %d---------------------------------\n", coult, vdwt);
            athread_init();
            //printf("Start slave fun......\n");
            athread_spawn(nbnxn_kernel_ElecQSTab_VdwLJ_VgrpF_ref_slave, 0);
            athread_join();
        ...
        else
        {
            ...
            //printf("---------------------------p_nbk_c_energrp, coult: %d, vdwt: %d---------------------------------\n", coult, vdwt);
            athread_init();
            //printf("Start slave fun......\n");
            athread_spawn(nbnxn_kernel_ElecQSTab_VdwLJ_VgrpF_ref_slave, 0);
            athread_join();
