...
gmx_inline static void calc_exponentials_q(int start, int end, real f, real *d, real *r, real *e)
{
    ...
    // {
    //     // printf("---------------------------calc_exponentials_q, d[%d]=%f---------------------------------\n", kx, d[kx]);
    //     d[kx] = 1.0/d[kx];
    //     // printf("---------------------------calc_exponentials_q, d[%d]=%f---------------------------------\n", kx, d[kx]);
    // }
    // for (kx = start; kx < end; kx++)
    // {
    //     // printf("---------------------------calc_exponentials_q, r[%d]=%f---------------------------------\n", kx, r[kx]);
    //     r[kx] = exp(r[kx]);
    //     // printf("---------------------------calc_exponentials_q, r[%d]=%f---------------------------------\n", kx, r[kx]);
    // }
    // for (kx = start; kx < end; kx++)
    // {
    //     // printf("---------------------------calc_exponentials_q, e[%d]=%f---------------------------------\n", kx, e[kx]);
    //     // e[kx] = f*r[kx]*d[kx];
    //     e[kx] = 0.0;
    //     // printf("---------------------------calc_exponentials_q, e[%d]=%f---------------------------------\n", kx, e[kx]);
    // }
                  ...
                  int nthread, int thread)
{
    ...
    //printf("---------------------------solve_pme_yzx gmx_parallel_3dfft_complex_limits: elfac=%.20f---------------------------------\n", elfac);
    ...
    //printf("---------------------------solve_pme_yzx gmx_parallel_3dfft_complex_limits---------------------------------\n");
    gmx_parallel_3dfft_complex_limits(pme->pfft_setup[PME_GRID_QA],
                                      complex_order,
                                      local_ndata,
                                      local_offset,
                                      local_size);
    //printf("---------------------------after solve_pme_yzx gmx_parallel_3dfft_complex_limits---------------------------------\n");
    ...
    for (iyz = iyz0; iyz < iyz1; iyz++)
    {
        ...
        if (bEnerVir)
        {
            ...
            //printf("---------------------------solve_pme_yzx calc_exponentials_q1, iyz0=%d, iyz1=%d, kxstart=%d, kxend=%d---------------------------------\n", iyz0, iyz1, kxstart, kxend);
            calc_exponentials_q(kxstart, kxend, elfac, denom, tmp1, eterm);
            /**
            float *eterm_tmp = (float *) malloc((kxend-kxstart) * sizeof(float));
            for (kx = kxstart; kx < kxend; kx++)
            {
                denom[kx] = 1.0/denom[kx];
                tmp1[kx] = exp(tmp1[kx]);
                real tt1 = tmp1[kx];
                tt1=((float)((int)((tt1+0.0000005)*1000000)))/1000000;
                // real t1 = elfac*tt1;
                real t1 = 0.01*0.01;
                t1=((float)((int)((t1+0.0000005)*1000000)))/1000000;
                // real t2 = denom[kx];
                real t2 = 0.01;
                t2=((float)((int)((t2+0.0005)*1000)))/1000;
                if (t1 > 0.000001)
                {
                    eterm_tmp[kx] = t1*t2;
                } else {
                    eterm_tmp[kx] = 0.000000;
                }
                eterm[kx] = 0.0;
                // eterm[kx] = elfac*tmp1[kx]*denom[kx];
                printf("---------------------------solve_pme_yzx calc_exponentials_q1, eterm[%d]=%.20f---------------------------------\n", kx, eterm[kx]);
            }
            free(eterm_tmp);
            */
            // printf("---------------------------after solve_pme_yzx calc_exponentials_q1---------------------------------\n");
        ...
        else
        {
            ...
            // printf("---------------------------solve_pme_yzx calc_exponentials_q2---------------------------------\n");
            calc_exponentials_q(kxstart, kxend, elfac, denom, tmp1, eterm);
            /**
            for (kx = kxstart; kx < kxend; kx++)
            {
                ...
                // eterm[kx] = elfac*tmp1[kx]*denom[kx];
                printf("---------------------------solve_pme_yzx calc_exponentials_q2, eterm[%d]=%.20f---------------------------------\n", kx, eterm[kx]);
            }
            */
            // printf("---------------------------after solve_pme_yzx calc_exponentials_q2---------------------------------\n");
